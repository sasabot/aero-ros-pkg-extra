...
from aero_maps import ColladaModel
...
class MovableModel(ColladaModel):
    ...
    ## @brief set pose of item[i] for callback,
    ##  this callback is NOT REGISTERED AUTOMATICALLY
    ##  because the class allows multiple instance generation.
    ##  To use this, please add manually in main function.
    ## @param msg aero_maps/IndexedPoseStamped,
    ##  if msg.header.frame_id = parent_id, call set_pose,
    ##  else call set_parent and set frame_id as new parent_id
    def set_indexed_pose_callback(self, msg):
        pos = (msg.pose.position.x,
               msg.pose.position.y,
               msg.pose.position.z)
        quat = (msg.pose.orientation.x,
                ...
                msg.pose.orientation.z,
                msg.pose.orientation.w)
        if self.props[msg.index]['parent_id'] == msg.header.frame_id:
            self.set_pose(msg.index, pos, quat)
        else:
            self.set_parent(msg.index, msg.header.frame_id, pos, quat)
